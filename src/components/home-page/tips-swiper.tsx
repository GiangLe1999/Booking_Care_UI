...
export default function TipsSwiper({ tips, isLoadingTips }: Props) {
  return (
        ...
        >
          {tips?.map((tip) => (
            <SwiperSlide key={tip.id}>
              <div>
                <Link to={`/danh-cho-bac-si/${tip?.slug}`} className="block">
                  <StyledImage
                    wrapperClasses="w-full aspect-[1.9] rounded-md border"
                    src={arrayBufferToBase64(tip?.thumbnail)}
                    alt={`Bài viết ${tip?.title}`}
                    imageClasses="rounded-md"
                  />
                </Link>
                <Link to={`/danh-cho-bac-si/${tip?.id}`} className="block">
