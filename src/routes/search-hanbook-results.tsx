...
import { getHandbookResults } from "../service/handbook.service";
...
const SearchHandbookResults: FC<Props> = (props): JSX.Element => {
  ...
  const fetchResults = async () => {
    if (query && query.length > 0) {
      setIsLoading(true);
      const data = await getHandbookResults(query);
      ...
      <div className="container my-10">
        <div className="flex items-center justify-between mt-10">
          <h1 className="font-semibold text-lg -mt-12">
            Tìm thấy {articles?.length || 0} bài viết Cẩm nang cho từ khóa "
            {query}"
          </h1>
...
export default SearchHandbookResults;
